...
public class MARS implements Algorithm {
    ...
    public static final int[] B = { 0xa4a8d57b, 0x5b5d193b, 0xc8a8309b, 0x73f9a978 };

    private static int S0(int x){ return S[x & 0xff]; }
    private static int S1(int x){ return S[256 + (x & 0xff)]; }
    private static int S9(int x){ return S[x & 0x1ff]; }
    ...
    public void setKey(byte[] key){
        ...
        for (int j = 0; j < 4; j++){
            for (int i = 0; i < 15; i++){
                int a = T[(i + 15 - 7) % 15];
                int b = T[(i + 15 - 2) % 15];
                T[i] ^= rotl(a ^ b, 3) ^ (4*i + j);
            }
            for (int r = 0; r < 4; r++){
                for (int i = 0; i < 15; i++){
                    T[i] = rotl(T[i] + S[(T[(i + 15 - 1) % 15]) & 0x1ff], 9);
                }
            }
            for (int i = 0; i < 10; i++){
                K[10*j + i] = T[(4*i) % 15];
    ...
    public byte[] decryptBlock(byte[] block){
        ...
        for (int i = 7; i >= 0; i--){
            ...
            A  = rotl(A, 24);
            D  = D ^ S1(rotr(A, 24));
            Cv = sub32(Cv, S0(rotr(A, 16)));
            Bv = sub32(Bv, S1(rotr(A, 8)));
            Bv = Bv ^ S0(A);
        }
    ...
    private static int add32(int a, int b){ return a + b; }
    private static int sub32(int a, int b){ return a - b; }
    private static int mul32(int a, int b){ return a * b; }

    private static int leToInt(byte[] b, int off){
        return  (b[off] & 0xff) |
                ((b[off+1] & 0xff) << 8) |
                ((b[off+2] & 0xff) << 16) |
                ((b[off+3] & 0xff) << 24);
    ...
    private static int genMask(int w){
        int M = (~w ^ (w >>> 1)) & 0x7fffffff;
        M &= (M >>> 1) & (M >>> 2);
        M &= (M >>> 3) & (M >>> 6);
        M <<= 1;
        M |= (M << 1);
